...
/**
 * @title GetUniswapV3TickDataBatchRequestBidirectional
 * @dev A utility contract designed to fetch initialized tick data from Uniswap V3 pools in both directions.
 *
 * This contract is not meant to be deployed. Instead, it should be used via a static call with the
 * deployment bytecode as payload. It retrieves tick data bidirectionally from a given tick,
 * collecting ONLY initialized ticks. It is a modified version of `GetUniswapV3TickDataBatchRequest`.
 *
 * Key features:
 * - Fetches active ticks in both directions from a starting tick
 * - Respects Uniswap V3's tick spacing and boundaries (MIN_TICK: -887272, MAX_TICK: 887272)
 * - Returns tick data including tick index and liquidity net liquidity
 * - Stops collecting when either max ticks are reached or liquidity becomes zero
 *
 * Usage:
 * This contract should be used through a static call to gather active tick data for applications
...
contract GetUniswapV3TickDataBatchRequestBidirectional {
    ...
    //    uint256 maxLoopCount = (3470 + uint256(uint24(tickSpacing*10))) / uint256(uint24(tickSpacing));
        ...
        while (loopCount < 1000 && counter < maxTicks) {
            loopCount++;
            if (loopCount % 1 == 0) {
