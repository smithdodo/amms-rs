 ...
 */
contract GetPancakeV3PoolDataBatchRequest {
    struct PoolData {
        address tokenA;
    ...
    constructor(address[] memory pools) {
        ...
        for (uint256 i = 0; i < pools.length; ++i) {
            ...
            poolData.tokenA = IPancakeV3Pool(poolAddress).token0();
            poolData.tokenB = IPancakeV3Pool(poolAddress).token1();
            ...
            (, int128 liquidityNet,,,,,,) = IPancakeV3Pool(poolAddress).ticks(tick);

            poolData.liquidity = IPancakeV3Pool(poolAddress).liquidity();
            poolData.tickSpacing = IPancakeV3Pool(poolAddress).tickSpacing();
            poolData.fee = IPancakeV3Pool(poolAddress).fee();
